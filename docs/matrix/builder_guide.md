# Matrix Builder Guide

This guide explains how to set up Matrix network infrastructure: relay nodes, routers, meatspace links, and networked devices.
...
device.db.acl = {char_pk: level}       # Access control list — dict of {pk: 1-10}
...
# Open the router navigation menu
route via CorpNet Router
# From the menu, you can browse access points and route to devices.
# Select the Corporate Plaza - Lobby AP, then select Executive Hub.
# This should create the ephemeral cluster and move you to its Checkpoint.
```
...
5. **Test as you go**: Jack in, use `route via <router>` and navigate to verify devices appear
